...
public class OrderDao extends BaseDAO<Order, ObjectId> {
    ...
    public PageModel<Order> findOrders(PageModel<Order> pageModel) {
        List<Order> orderList = find(createQuery().filter("adminUserId", new ObjectId("570c6708a276ee72e9277424")).order("dateUpd").offset(pageModel.getPageNo() * pageModel.getPageSize()).limit(pageModel.getPageSize())).asList();
