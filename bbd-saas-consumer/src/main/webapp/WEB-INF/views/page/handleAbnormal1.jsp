...
function gotoPage(pageIndex) {
	...
	$.ajax({
		type : "GET",  //提交方式  
        url : "<%=path%>/handleAbnormal1/getList",//路径
        data : {  
            "pageIndex" : pageIndex,
            "status" : $("#status").val(), 
...
function getRowHtml(data){
	...
	//派件员==未分派，不需要显示派件员姓名和电话
	 ...
	 if(data.orderStatus == "<%=OrderStatus.RETENTION %>" || data.orderStatus==null){
		...
		row += "<td class='tl' width='190px'><a href='javascript:void(0);' onclick='showOtherSiteDiv(\"" + data.mailNum + "\")' class='orange'>转其他站点</a>";
...
function initCourierList() {
	...
	$.ajax({
		type : "GET",  //提交方式  
        url : "<%=path%>/handleAbnormal1/getAllUserList",//路径
...
function chooseCourier() {
	...
	$.ajax({
		type : "GET",  //提交方式  
        url : "<%=path%>/handleAbnormal1/reDispatch",//路径
		dataType: "json",
        data : {  
...
function initSiteList() {
	...
	$.ajax({
		type : "GET",  //提交方式  
        url : "<%=path%>/handleAbnormal1/getAllOtherSiteList",//路径
        data : {},//数据，这里使用的是Json格式进行传输  
        success : function(dataList) {//返回数据根据结果进行相应的处理  
        	siteList = dataList;
        },
...
function chooseOtherSite() {
	...
	$.ajax({
		type : "GET",  //提交方式
		url : "<%=path%>/handleAbnormal1/toOtherSite",//路径
		data : {
			"mailNum" : mailNum, //
			"siteId" : siteId,//站点编号
...
function showApplyReturnDiv(mailNumStr) {
	...
	$.ajax({
		type : "GET",  //提交方式
		url : "<%=path%>/handleAbnormal1/getRtnReasonList",//路径
		data : {},//数据，这里使用的是Json格式进行传输
		success : function(dataList) {//返回数据根据结果进行相应的处理
...
function applyReturn() {
	...
	$.ajax({
		type : "POST",  //提交方式
		url : "<%=path%>/handleAbnormal1/doReturn?${_csrf.parameterName}=${_csrf.token}",//路径
		data : {
			"mailNum" : mailNum, //
			"rtnReason" : rtnReason,
			"rtnRemark" : rtnRemark,
...
function initExpressCompanys() {
	...
	$.ajax({
		type : "GET",  //提交方式
		url : "<%=path%>/handleAbnormal1/getExpressCompanys",//路径
		data : {},//数据，这里使用的是Json格式进行传输
		success : function(dataList) {//返回数据根据结果进行相应的处理
...
function toOtherExpressCompanys() {
	...
	$.ajax({
		type: "POST",  //提交方式
		url: "<%=path%>/handleAbnormal1/toOtherExpressCompanys",//路径
		data: {
			"mailNum": mailNum, //运单号
			"companyId": companyId,
